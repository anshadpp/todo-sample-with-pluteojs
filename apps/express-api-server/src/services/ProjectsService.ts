import {db, eq, and, or, isNull, projects, boards, categories, members} from "@pluteojs/database";

import logger from "@loaders/logger";

import {httpStatusCodes} from "@customTypes/networkTypes";
import type {iProject, iCreateProjectDTO, iUpdateProjectDTO} from "@customTypes/appDataTypes/projectTypes";
...
export default class ProjectsService {
	...
	public async createProject(userId: string, organizationId: string | null, input: iCreateProjectDTO): Promise<iProject> {
		...
		// Create a default board with default categories
		...
		const defaultCategories = [
			{name: "Todo", color: "#6B7280", sortOrder: 0},
			{name: "In Progress", color: "#3B82F6", sortOrder: 1000},
			{name: "Review", color: "#F59E0B", sortOrder: 2000},
			{name: "Done", color: "#10B981", sortOrder: 3000},
		];
		...
		logger.silly("Project created successfully with default board and categories");
	 ...
	 */
	public async getProjects(userId: string, organizationId: string | null): Promise<iProject[]> {
		logger.silly("Retrieving projects");

		let condition;
		if (organizationId) {
			// Org projects: projects belonging to this org
			condition = and(eq(projects.organizationId, organizationId), eq(projects.isArchived, false));
		} else {
	...
	public async getProject(projectId: string, userId: string): Promise<iProject> {
		...
		if (record.organizationId) {
			const isMember = await this.verifyMembership(userId, record.organizationId);
	...
	public async updateProject(projectId: string, userId: string, input: iUpdateProjectDTO): Promise<iProject> {
	...
	public async verifyMembership(userId: string, organizationId: string): Promise<boolean> {
		const result = await db
			.select()
			.from(members)
