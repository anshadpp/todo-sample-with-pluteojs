import {db, eq, and, desc, asc, tasks, users, labels, taskLabels, taskDependencies, isNull} from "@pluteojs/database";

import logger from "@loaders/logger";

import {httpStatusCodes} from "@customTypes/networkTypes";
import type {iTask, iCreateTaskDTO, iUpdateTaskDTO, iMoveTaskDTO, iTaskDependency} from "@customTypes/appDataTypes/taskTypes";
...
export default class TasksService {
	...
	public async createTask(userId: string, projectId: string, input: iCreateTaskDTO): Promise<iTask> {
	...
	public async updateTask(taskId: string, input: iUpdateTaskDTO): Promise<iTask> {
		...
		if (input.assigneeId !== undefined) {updateValues.assigneeId = input.assigneeId;}
		...
		if (input.startAt !== undefined) {updateValues.startAt = input.startAt ? new Date(input.startAt) : null;}
		if (input.estimatedMinutes !== undefined) {updateValues.estimatedMinutes = input.estimatedMinutes;}
		if (input.effortLevel !== undefined) {updateValues.effortLevel = input.effortLevel;}
		if (input.coverImage !== undefined) {updateValues.coverImage = input.coverImage;}
		if (input.isArchived !== undefined) {updateValues.isArchived = input.isArchived;}
	...
	public async reorderTasks(items: {id: string; sortOrder: number}[]): Promise<void> {
	...
	public async addDependency(
		...
		if (dependentTaskId === dependsOnTaskId) {
			throw new ServiceError(
				...
				{error: "SelfDependency", message: "A task cannot depend on itself", details: null},
			);
		}

		// Check for circular dependency
		const wouldBeCyclic = await this.wouldCreateCycle(dependentTaskId, dependsOnTaskId);
		if (wouldBeCyclic) {
			throw new ServiceError(
				httpStatusCodes.CLIENT_ERROR_BAD_REQUEST,
				{error: "CircularDependency", message: "This would create a circular dependency", details: null},
			);
		}
		...
		const depTask = await db
			.select({id: tasks.id, title: tasks.title, status: tasks.status, categoryId: tasks.categoryId})
			.from(tasks)
	...
	public async removeDependency(dependencyId: string): Promise<void> {
		...
		if (result.length === 0) {
			throw new ServiceError(
				httpStatusCodes.CLIENT_ERROR_NOT_FOUND,
				{error: "DependencyNotFound", message: "Dependency not found", details: null},
			);
		}
	...
	public async getDependencies(taskId: string): Promise<{
		...
		return {
			blockedBy: blockedByRaw.map((r) => {return {
				id: r.id,
				dependentTaskId: r.dependentTaskId,
				dependsOnTaskId: r.dependsOnTaskId,
				dependencyType: r.dependencyType,
				createdAt: r.createdAt.toISOString(),
				dependsOnTask: {id: r.taskId, title: r.taskTitle, status: r.taskStatus, categoryId: r.taskCategoryId},
			};}),
			blocking: blockingRaw.map((r) => {return {
				id: r.id,
				dependentTaskId: r.dependentTaskId,
				dependsOnTaskId: r.dependsOnTaskId,
				dependencyType: r.dependencyType,
				createdAt: r.createdAt.toISOString(),
				dependentTask: {id: r.taskId, title: r.taskTitle, status: r.taskStatus, categoryId: r.taskCategoryId},
			};}),
		};
	}
	 ...
	 */
	private async wouldCreateCycle(dependentTaskId: string, dependsOnTaskId: string): Promise<boolean> {
		const visited = new Set<string>();
		const queue = [dependentTaskId];

		while (queue.length > 0) {
			const current = queue.shift()!;
			if (current === dependsOnTaskId) {continue;} // skip the direct edge we're adding
			...
			for (const b of blocked) {
				if (b.dependentTaskId === dependsOnTaskId) {return true;}
