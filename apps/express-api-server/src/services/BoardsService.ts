...
import type {iBoard, iCreateBoardDTO, iUpdateBoardDTO, iCategory, iCreateCategoryDTO, iUpdateCategoryDTO} from "@customTypes/appDataTypes/projectTypes";
...
export default class BoardsService {
	...
	public async getBoards(projectId: string): Promise<iBoard[]> {
		...
		return records.map((r) => {return this.toBoardDTO(r);});
	}

	public async createBoard(projectId: string, input: iCreateBoardDTO): Promise<iBoard> {
	...
	public async updateBoard(boardId: string, input: iUpdateBoardDTO): Promise<iBoard> {
	...
	public async getCategories(boardId: string): Promise<iCategory[]> {
		...
		return records.map((r) => {return this.toCategoryDTO(r);});
	}

	public async createCategory(boardId: string, input: iCreateCategoryDTO): Promise<iCategory> {
		...
		const maxOrder = existing.length > 0 ? existing[existing.length - 1]!.sortOrder : -1000;
	...
	public async updateCategory(categoryId: string, input: iUpdateCategoryDTO): Promise<iCategory> {
	...
	public async reorderCategories(boardId: string, items: {id: string; sortOrder: number}[]): Promise<iCategory[]> {
		...
		for (const item of items) {
			await db
				...
				.where(and(eq(categories.id, item.id), eq(categories.boardId, boardId)));
		}
